...
import { ArrowLeft, Plus, Pencil, Trash2, X, Save, Loader2, Upload, Users } from "lucide-react";

export default function PemerintahanEditor() {
  ...
  const fetchItems = async () => {
    const { data } = await supabase.from("pemerintahan").select("*").order("sort_order", { ascending: true });
    setItems(data || []);
    setLoading(false);
  };
  ...
  const openAdd = () => { setEditingItem(null); setForm({ nama: "", jabatan: "", foto_url: "" }); setModalOpen(true); };
  const openEdit = (item) => { setEditingItem(item); setForm({ nama: item.nama, jabatan: item.jabatan, foto_url: item.foto_url || "" }); setModalOpen(true); };
  ...
  const handlePhotoUpload = async (e) => {
    ...
    const { error } = await supabase.storage.from("staff-photos").upload(fileName, file, { upsert: true });
    if (error) {
      showMessage("error", "Gagal mengunggah foto: " + error.message);
      setUploading(false);
      return;
    }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (editingItem) {
      const { error } = await supabase.from("pemerintahan").update({ nama: form.nama, jabatan: form.jabatan, foto_url: form.foto_url || null }).eq("id", editingItem.id);
      ...
      const { error } = await supabase.from("pemerintahan").insert({ nama: form.nama, jabatan: form.jabatan, foto_url: form.foto_url || null, sort_order: maxOrder + 1 });
  ...
  const handleDelete = async (id) => {
    const { error } = await supabase.from("pemerintahan").delete().eq("id", id);
  ...
  const moveItem = async (index, direction) => {
    ...
    await Promise.all([
      supabase.from("pemerintahan").update({ sort_order: index }).eq("id", newItems[index].id),
      supabase.from("pemerintahan").update({ sort_order: swapIndex }).eq("id", newItems[swapIndex].id),
    ]);
    fetchItems();
  };
    ...
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Struktur Pemerintahan</h1>
            <p className="text-sm text-slate-500">{items.length} perangkat desa</p>
          </div>
        </div>
      ...
      <div className="space-y-3">
        {items.map((item, index) => (
          <div key={item.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="text-xs text-emerald-600 font-medium">{item.jabatan}</span>
            </div>
        ...
        {items.length === 0 && <div className="text-center py-12 text-sm text-slate-400">Belum ada data.</div>}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-slate-800">{editingItem ? "Edit" : "Tambah"} Perangkat</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={form.nama} onChange={(e) => setForm({ ...form, nama: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Jabatan</label>
                <input type="text" value={form.jabatan} onChange={(e) => setForm({ ...form, jabatan: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Foto (opsional)</label>
                {form.foto_url && (
                  <div className="relative mb-2 w-20 h-20 rounded-xl overflow-hidden border border-slate-200">
                    <img src={form.foto_url} alt="" className="w-full h-full object-cover" />
                    <button onClick={() => setForm({ ...form, foto_url: "" })} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-md"><Trash2 className="w-3 h-3" /></button>
                  </div>
                )}
                <label className="flex items-center gap-2 cursor-pointer bg-slate-50 hover:bg-slate-100 border border-slate-200 border-dashed rounded-xl px-4 py-2.5 transition-colors">
                  ...
                  <span className="text-sm text-slate-600">{uploading ? "Mengunggah..." : "Pilih foto..."}</span>
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button onClick={handleSave} disabled={saving || !form.nama.trim() || !form.jabatan.trim()} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm px-4 py-2 rounded-xl disabled:opacity-60">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-800 mb-2">Hapus Perangkat?</h3>
